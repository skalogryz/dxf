...
// todo: move it to dxf parseutils
  ...
  *)
  ...
  TDxfPlotSettings = class(TDxfObject)
    SubClass2      : string;  // Subclass marker (AcDbPlotSettings)
    PageName       : string;  //   1 Page Setup name
    PrinterName    : string;  //   2 Name of system printer or plot configuration file
    PaperSize      : string;  //   4 Paper size
    PlotViewName   : string;  //   6 Plot view name
    MarginLeftMm   : double;  //  40 Size, in millimeters, of unprintable margin on left side of paper
    MarginBottomMm : double;  //  41 Size, in millimeters, of unprintable margin on bottom of paper
    MarginRightMm  : double;  //  42 Size, in millimeters, of unprintable margin on right side of paper
    MarginTopMm    : double;  //  43 Size, in millimeters, of unprintable margin on top of paper
    WidthMm        : double;  //  44 Plot paper size: physical paper width in millimeters
    HeightMm       : double;  //  45 Plot paper size: physical paper height in millimeters
    OffsetX        : double;  //  46 Plot origin: X value of origin offset in millimeters
    OffsetY        : double;  //  47 Plot origin: Y value of origin offset in millimeters
    PlotX          : double;  //  48 Plot window area: X value of lower-left window corner
    PlotY          : double;  //  49 Plot window area: Y value of upper-right window corner
    PlotX2         : double;  // 140 Plot window area: X value of lower-left window corner
    PlotY2         : double;  // 141 Plot window area: Y value of upper-right window corner
    Numerator      : double;  // 142 Numerator of custom print scale: real world (paper) units
    Denominator    : double;  // 143 Denominator of custom print scale: drawing units
    Flags          : integer; //  70 Plot layout flag:
                              ...
                              //     8192 = Initializing
                              //     16384 = PrevPlotInit
    PlotUnits      : integer; //  72 Plot paper units:
                              //     0 = Plot in inches
                              //     1 = Plot in millimeters
                              //     2 = Plot in pixels
    PlotRotation   : integer; //  73 Plot rotation:
                              //     0 = No rotation
                              //     1 = 90 degrees counterclockwise
                              //     2 = Upside-down
                              //     3 = 90 degrees clockwise
    PlotType       : integer; //  74 Plot type (portion of paper space to output to the media):
    ...
    ShadePlotMode  : integer; //  76 ShadePlot mode:
                              //     0 = As Displayed
                              //     1 = Wireframe
                              //     2 = Hidden
                              //     3 = Rendered
    ShadePlotRes   : integer; //  77 ShadePlot resolution level:
                              //     0 = Draft
                              //     1 = Preview
                              //     2 = Normal
                              //     3 = Presentation
                              //     4 = Maximum
                              //     5 = Custom
    ShadePlotDPI   : integer; //  78 ShadePlot custom DPI:
                              //     Valid range: 100 to 32767
                              //     Only applied when the ShadePlot resolution level is set to 5 (Custom)
    ViewScale      : double;  // 147 A floating point scale factor that represents the standard scale value
  ...
  //LAYOUT
  //
  TDxfLayout = class(TDxfPlotSettings)
    SubClass3  : string;      // 100 Subclass marker (DictionaryVariables)
    LayoutName : string;      //   1 Layout name
    LayoutFlags: Integer;     //  70 Flag (bit-coded) to control the following:
